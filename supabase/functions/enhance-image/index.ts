              ...
              content: [
                {
                  type: "text",
                  text: (customPrompt
                    ? `CRITICAL: The output image MUST be EXACTLY ${width || "the same"}x${height || "the same"} pixels — the EXACT same dimensions as the input. Do NOT crop, resize, or add padding. Preserve transparent areas (alpha=0). Output as PNG.\n\n${customPrompt}`
                    : `This is a PNG image${width && height ? ` of ${width}x${height} pixels` : ""} with transparent areas (alpha channel). STRICT RULES — violating ANY of these means failure: 1) Output a PNG at EXACTLY ${width || "the same"}x${height || "the same"} pixel dimensions. 2) Transparent pixels MUST stay fully transparent (alpha=0). NO background added. 3) DO NOT CHANGE ANY HUES. Do NOT introduce new colors. Do NOT shift colors (e.g. blue to purple, red to orange, green to teal). The ONLY modification allowed is increasing saturation and brightness of the EXISTING exact colors. Imagine converting each pixel to HSL, keeping H unchanged, and increasing S and L slightly. That is ALL you may do. 4) Do not redraw, reinterpret, or reimagine the artwork. Copy it pixel-perfectly with only saturation/brightness boosted. 5) No cropping, resizing, padding, borders, text, or watermarks. 6) Alpha channel must be identical to input. This is for NFT trait layers.`),
                },
                {
                  type: "image_url",
                  image_url: {
